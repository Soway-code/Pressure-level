...
const uint8_t encryptionString[64] = "ShenZhen Soway Science and Technology Development Co., Ltd.    ";
...
//  HAL_UART_Transmit(&UartHandle, p_string, length, TX_TIMEOUT);
  ...
  BSP_UART_Transmit_DMA(p_string, length);
}
...
/**
  ...
  */
uint8_t Serial_PutByte( uint8_t param )
{
//  return HAL_UART_Transmit(&UartHandle, &param, 1, TX_TIMEOUT);
    while(Sensor_USART_Get_TX_Cplt_Flag() == 0);
    Sensor_USART_Clear_TX_Cplt_Flag();
    
    return BSP_UART_Transmit_DMA(&param, 1);        
}
/**
  * @}
  */
