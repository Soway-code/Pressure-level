...
#ifdef RT_ADC_USING_DMA
static rt_size_t _adc_read_dma(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    rt_err_t result = RT_EOK;
    struct rt_adc_device *adc = (struct rt_adc_device *)dev;
    rt_uint32_t *value = (rt_uint32_t *)buffer;

    result = adc->ops->convert_dma(adc, pos, value, size);
    if (result != RT_EOK)
    {
        return 0;
    }
...
static rt_err_t _adc_open(rt_device_t dev, rt_uint16_t oflag)
{    
    if((oflag & RT_DEVICE_OFLAG_RDONLY) == 0)
    {
        return -RT_ERROR;
    }
