...
#define REG2_CMEAS_PC2_EN                   (0x03 << 16)/* 使能PC2端口 */  
#define REG2_CMEAS_PC3_EN                   (0x04 << 16)/* 使能PC3端口 */  
...
#define REG6_TMEAS_FAKE_8_TIMES             (0x01 << 15)/* 2 次假测量 */

/* 芯片内部集成了一个温敏电阻和一个内部参考电阻.另外还提供了连接外部温度传感器和参
考电阻的端口.总而言之,这颗芯片支持11种组合.但是在这里所描述的带有标准固件的芯片,
将会仅有6种组合可能: */
/* Res11 = R(内部铝电阻)/R(内部多晶硅电阻) */
#define REG6_TMEAS_7BITS_AL_SI              0x43        
/* Res10 = R(外部电阻)/R(内部多晶硅电阻)，Res11 = R(内部铝电阻)/R(内部多晶硅电阻) */
#define REG6_TMEAS_7BITS_EX_SI_AL_SI        0x4B        
/* Res10 = R(外部电阻)/R(内部多晶硅电阻) */
#define REG6_TMEAS_7BITS_EX_SI              0x49        
/* Res11 = R(内部铝电阻)/R(外部参考电阻) */
#define REG6_TMEAS_7BITS_AL_REF             0x06        
/* Res10 = R(外部电阻)/R(外部参考电阻)，Res11 = R(内部铝电阻)/R(外部参考电阻) */
#define REG6_TMEAS_7BITS_EX_REF_AL_REF      0x0E        
/* Res10 = R(外部电阻)/R(外部参考电阻) */
#define REG6_TMEAS_7BITS_EX_REF             0x0C        
...
#define REG10_PV_CORE_CTL_STANDARD          0x180047    /* 标准 */
#define REG10_V_CORE_CTL_LOW_POWER          0x180087    /* 低功耗 */
...
/* REG7寄存器值，固定0xC71F0000 */
#define REG7_CONFIG_VALUE                   0xC71F0000
...
/* REG10寄存器值 */
#define REG10_IN_1_8V_REGULATOR_VALUE       /* REG10，控制1.8 V 核心电压调节器，低功耗的设置也可以获得好的测量精   \
                                            度,但是这个与RDC的测量速度和时序有关.请参考附录当中的例子 */          \
...
//SPI片选端口时钟
#define SPI_NSS_GPIO_CLK()                              __HAL_RCC_GPIOA_CLK_ENABLE()

//SPI使能
#define PCAP_SPI_ENABLE                     HAL_GPIO_WritePin(SPI_NSS_GPIO_PORT, SPI_NSS_GPIO_PIN, GPIO_PIN_RESET)
//SPI禁止
#define PCAP_SPI_DISABLE                    HAL_GPIO_WritePin(SPI_NSS_GPIO_PORT, SPI_NSS_GPIO_PIN, GPIO_PIN_SET)
